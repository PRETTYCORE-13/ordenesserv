defmodule PrettycoreWeb.ClienteFormLive do
  ...
  @impl true
  def handle_event("estado_change", %{"_target" => target, "cliente_form" => params}, socket) do
    # Extract direccion index from target path
    # Target path: ["cliente_form", "direcciones", "0", "mapedo_codigo_k"]
    direccion_index = Enum.at(target, 2)

    # Get estado_codigo from the direccion at the index
    estado_codigo = get_in(params, ["direcciones", direccion_index, "mapedo_codigo_k"])
  ...
  @impl true
  def handle_event("municipio_change", %{"_target" => target, "cliente_form" => params}, socket) do
    # Extract direccion index from target path
    # Target path: ["cliente_form", "direcciones", "0", "mapmun_codigo_k"]
    direccion_index = Enum.at(target, 2)

    # Get estado_codigo and municipio_codigo from the direccion at the index
    estado_codigo = get_in(params, ["direcciones", direccion_index, "mapedo_codigo_k"])
    municipio_codigo = get_in(params, ["direcciones", direccion_index, "mapmun_codigo_k"])
